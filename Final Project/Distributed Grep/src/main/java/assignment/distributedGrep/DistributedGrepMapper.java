...
public class DistributedGrepMapper extends Mapper<LongWritable, Text, Text, BusinessCityTuple> {
	...
	protected void map(LongWritable key, Text value,
		...
		if (key.get() == 0) {
			return;
		} else if ((fields[12].matches(".*Italian.Restaurants.*") && fields[4].equals("Tempe"))
				|| (fields[12].matches(".*Indian.Restaurants.*") && fields[4].equals("Tempe"))
				|| (fields[12].matches(".*Chinese.Restaurants.*") && fields[4].equals("Tempe"))
				|| (fields[12].matches(".*Mexican.Restaurants.*") && fields[4].equals("Tempe"))) {
